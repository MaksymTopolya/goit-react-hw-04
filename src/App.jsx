...
function App() {
  ...
  const [selectedImage, setSelectedImage] = useState({img: null, alt: null});
  ...
  useEffect(() => {
    if (inputValue) {
      ...
      setIsLoading(true);
      searchCollections(inputValue, page)
        .then((data) => {
          setIsError(false);
  ...
  return (
    <div>
      ...
      <ImageModal isOpen={modalIsOpen} onOpen={setIsOpen} image={selectedImage} />
    </div>
  );
}
